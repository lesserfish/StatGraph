run_plot <- function(model, 
  ...
  legend("topright",  inset = c(-0.29, 0), # Position of the legend
...
Cairo::CairoSVG(file = "out/model-selection_ER.svg",height = 5, onefile = TRUE, bg = "transparent", pointsize = 12)
par(mfrow = c(1, 1))
...
Cairo::CairoSVG(file = "out/model-selection_PA.svg",height = 5, onefile = TRUE, bg = "transparent", pointsize = 12)
par(mfrow = c(1, 1))
run_plot("PAP", main = "Barabási–Albert Model")
dev.off()

Cairo::CairoSVG(file = "out/model-selection_WS.svg",height = 5, onefile = TRUE, bg = "transparent", pointsize = 12)
par(mfrow = c(1, 1))
