...
graph.model.selection <- function(G, models=NULL, parameters=NULL, eps=0.01,
                                  bandwidth="Silverman", eigenvalues=NULL, directed=FALSE, ...){

    # TODO: Validate parameters for the directed case.
    ...
    p <- NULL
    # TODO: Add option to send eigenvalues
    for(i in 1:length(models)){
        if(!is.null(parameters)) p <- parameters[[i]]
        r <- graph.param.estimator(G=A, model=models[[i]], parameters=p, bandwidth=bandwidth, directed=directed, search_mode="grid_search", ...)
        results[i, "param"] <- r$p
        results[i, "GIC"]   <- r$KLD
    }
